...
double akima_periodic_gsl::operator()(double x) const {
  return gsl_spline_eval(spline_, x, acc_);
}
double akima_periodic_gsl::derivative(double x) const {
  return  gsl_spline_eval_deriv(spline_, x, acc_);
}
double akima_periodic_gsl::derivative2(double x) const {
